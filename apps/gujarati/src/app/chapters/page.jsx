                            ...
                            >
                                ...
                                <div className="chapter-actions">
                                    <button
                                        ...
                                        onClick={() => router.push(`/theory/${chapter.id}`)}
                                    >
                                        Theory
                                    </button>
                                    <button
                                        className="action-btn btn-test"
                                        onClick={() => router.push(`/test/${chapter.id}`)}
                                    >
                                        Test
                                    </button>
                                    <button
                                        className="action-btn btn-pdf"
                                        onClick={() => router.push(`/pdf-view?file=${encodeURIComponent(`/pdf/Ch ${chapter.id} theory.pdf`)}&title=${encodeURIComponent(`Ch ${chapter.id} - ${chapter.name} (Theory)`)}`)}
                                    >
                                        Theory PDF
                                    </button>
                                    {chapter.hasTestPdf && (
                                        <button
                                            className="action-btn btn-pdf"
                                            onClick={() => router.push(`/pdf-view?file=${encodeURIComponent(`/pdf/Ch ${chapter.id} questions.pdf`)}&title=${encodeURIComponent(`Ch ${chapter.id} - ${chapter.name} (Test)`)}`)}
                                        >
                                            Test PDF
                                        </button>
                                    )}
