...
export default function PdfViewer() {
    ...
    const pdfUrlRaw = searchParams.get('file');
    const pdfUrl = React.useMemo(() => {
        if (!pdfUrlRaw) return null;
        // In production, Next.js assets are served from the /gujarati basePath.
        // We need to prepend it if it's missing for the PDF fetch to work.
        const isProduction = process.env.NODE_ENV === 'production';
        if (isProduction && pdfUrlRaw.startsWith('/') && !pdfUrlRaw.startsWith('/gujarati')) {
            return `/gujarati${pdfUrlRaw}`;
        }
        return pdfUrlRaw;
    }, [pdfUrlRaw]);
    ...
    if (!pdfUrl) {
        return (
            <div style={{ textAlign: 'center', padding: '50px', color: '#ef4444' }}>
                <h2>PDF not found!</h2>
                <button onClick={() => router.push('/chapters')} style={{
